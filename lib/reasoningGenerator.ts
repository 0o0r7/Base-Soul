...
export function generateReasoning(
  ...
  const castPercentile = Math.round(signals.castCount * 100);
  const long = `The Base Soul Oracle has analyzed your presence across your recent casts. Your soul resonates with ${dimDescriptor} ${topSignals[0]} and ${topSignals[1]}, patterns that reveal the essence of ${archetype.name}. ${archetype.traits.join(', ')}—shine through in how you engage with the Farcaster ecosystem. This reading is derived from real behavioral signals. This is not random. This is you.`;
